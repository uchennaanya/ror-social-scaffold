  ...
  def show_buttons(user)
    ...
    if user.id == current_user.id
      ...
      if request_sent(user)
       ...
       result += "#{link_to 'Accept friend request',  update_request_path(user: current_user.id, friend: user, status: 'accept'),
        class: 'profile-link'}
        <span>&nbsp</span>
        <span>&nbsp</span>
       ...
       result += "#{link_to 'Invite friend',  friendships_path(user: @current_user, friend: user), :method => :post,
        class: 'profile-link'}".html_safe
      end
      result.html_safe
    end
